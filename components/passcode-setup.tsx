...
import { isBiometricAvailable, registerBiometric } from "@/lib/biometrics"
...
export default function PasscodeSetup({ userId, onComplete }: PasscodeSetupProps) {
  ...
  const handleBiometricSetup = async () => {
    setIsProcessing(true)
    const success = await registerBiometric(userId) // Using userId as name for privacy/simplicity or fetch username
    if (success) {
      localStorage.setItem(`biometric_enabled_${userId}`, "true")
      onComplete()
    } else {
      setError("Failed to register Face ID. You can rely on your passcode.")
      setIsProcessing(false)
      // Allow skipping
    }
  }
      ...
      <div className={`z-10 w-full max-w-sm mx-4 ${shakeAnimation ? "animate-shake" : ""}`}>
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-3xl font-bold bg-gradient-to-r from-white via-blue-100 to-purple-100 bg-clip-text text-transparent mb-3">
            {step === "create" ? "Create Security Code" : step === "confirm" ? "Confirm Security Code" : "Enable Face ID"}
          </h1>
        ...
        {step === "biometric" ? (
          <div className="space-y-4">
            <button
              ...
              className="w-full py-4 px-6 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl flex items-center justify-center transition-all"
            >
            ...
            <div className="space-y-4">
              {step === "create" ? (
                ...
                <button
                  ...
                  className="w-full py-4 px-6 text-center text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 rounded-2xl border border-emerald-500/50 hover:border-emerald-400/50 transition-all duration-200 ease-out transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none shadow-lg shadow-emerald-500/25 font-semibold"
                >
                  {isProcessing ? (
